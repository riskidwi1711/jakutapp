...
function Dasawisma(props) {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Inertia.delete(`/delete/korwe/${id}`, {
          onSuccess: () => {
  ...
  const ExpandedComponent = ({ data }) => (
    <div className="py-3">
      ...
      <p>
        Kecamatan :{" "}
        {props.kecamatan.find((e) => e.id == data.kecamatan_id).nama}
      </p>
      <p>
        Kelurahan :{" "}
        {props.kelurahan.find((e) => e.id == data.kelurahan_id).nama}
      </p>
      <p>RT : {data.rt}</p>
      <p>RW : {data.rw}</p>
